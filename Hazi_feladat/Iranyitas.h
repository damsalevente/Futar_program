...
public:
	const static int AutoDist =30;
	const static int TeherDist=60;
	Futar() {};
...
public:
	...
	//Ha 30km-nél távolabb van, +1, ha 60 km-nél távolabbra kell szállítani +2
...
private:
	...
	//dinamikus tömb
	int mNum;
	//megrendelések darabszáma (ez a háziban dinamikus adattag 
	int num;
	//futárok száma, még mielõtt át nem írtam vectorrá
public:
	Ceg();
	...
	//visszaadja a futár vector-t 
	void step();
	//Ezzel fogom léptetni a cég futar vector futarjainak a helyet
	//idõben léptetés: a futárok, akik rendelkeznek csomagokkal átugranak a destx destY pontra, size pedig -1-re megy,így látom hogy ez már befutott csomag 
	operator int();
	//ha valahol intként kell kezelni, igazából csak a hazi követelmények miatt van benne
};
bool operator <(const Futar & first, const Futar & second);
//két futárt az azonosítója alapján vetünk össze 
ostream &operator <<(ostream & o, const Futar &a);
//kiíárs
template<typename T>
void printer(const T & a )
{
	cout << a;
}
//mindenféle dolgot kiír a standard kimenetre, template függvény
#endif